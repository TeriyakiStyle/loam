import { prospectSVG, runProspect, BANDS, PARTICLES } from '../art/prospect.js';
export function render(el){
 ...
 <figcaption><span class="pj-key"><i></i>Food available</span><span class="pj-key needs"><i></i>Daily needs</span>${PARTICLES.map(p=>`<span class="pj-key dot"><i style="background:${p.tone}"></i>${p.label}</span>`).join('')}</figcaption></figure>
